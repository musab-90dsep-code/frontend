...
const Carousel = ({ videos }) => {
  ...
  // --- আপডেট ১: ইউটিউব এবং ক্লাউডিনারি উভয়ের জন্য থাম্বনেইল বের করার ফাংশন ---
  const getVideoThumbnail = (url) => {
    if (!url) return "https://via.placeholder.com/640x360?text=No+Video";

    // ১. যদি ক্লাউডিনারির ভিডিও বা ডাইরেক্ট .mp4 ভিডিও হয়, তাহলে নামের শেষে .jpg বসিয়ে থাম্বনেইল আনবে
    if (url.match(/\.(mp4|webm|ogg|mov)$/i)) {
      return url.replace(/\.(mp4|webm|ogg|mov)$/i, '.jpg');
    }

    // ২. যদি ইউটিউব ভিডিও হয়
  ...
  // ইউটিউবের এমবেড লিংক জেনারেট করার জন্য
          ...
          <SwiperSlide key={idx}>
            ...
            >
              <div className="rounded-[2rem] overflow-hidden shadow-lg border border-gray-100 aspect-video relative group-hover:shadow-emerald-100 transition-all duration-500">
                <img 
                  // আপডেট: getVideoThumbnail কল করা হয়েছে
                  src={video.image_url 
      ...
      <AnimatePresence>
        {selectedVideo && (
          ...
          >
            ...
            >
              <button 
                className="absolute top-5 right-5 z-10 p-2.5 bg-white/10 hover:bg-white/20 rounded-full text-white transition-colors z-50"
                onClick={() => setSelectedVideo(null)}
              >
                <X size={24} />
              </button>

              {/* --- আপডেট ২: ভিডিও প্লেয়ার লজিক --- */}
              {selectedVideo.video_url && selectedVideo.video_url.match(/\.(mp4|webm|ogg|mov)$/i) ? (
                // যদি ক্লাউডিনারি বা সরাসরি ভিডিও ফাইল হয়
                <video
